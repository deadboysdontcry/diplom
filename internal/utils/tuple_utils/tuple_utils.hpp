...
namespace nTupleUtils {
    ...
    namespace {
        ...
        template <typename Array, typename IdxsTuple, typename... Types>
        struct ReadToTuple;
        ...
        public:
            ReadToTuple(Array const& a, IndexesTuple<Idxs...> idx)
                : t(GetFromStringFromArray<Types>(a, Idxs)...)
            {
            }
        ...
        template <typename Tuple, typename Array, typename Indexs>
        struct callReadToTuple;

        template <typename... Types, typename Array, typename Indexs>
        struct callReadToTuple<std::tuple<Types...>, Array, Indexs> {
            callReadToTuple(Array& aFs, Indexs& aInd)
                : r(aFs, aInd)
            {
            }
